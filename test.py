...
path2 = "logs/fedstream/pre_estimate_theta_best_sigma:0.75.npy"
result = np.load(path2)
reward = np.array(result[0][0])
theta = np.array(result[0][1])
cost = np.array(result[1][0])

path_3 = "logs/fedstream/client5_round50_initdata700/pre/standard.npz"
np.savez(path_3, phi=phi_list, reward=reward, theta=theta, cost=cost)
...
np.savez("myfile.npz", A=a, B=b, C=c)
...
transform = transforms.Compose(
    [transforms.ToTensor(), transforms.Normalize((0.13065973,), (0.3015038,))]
)
...
# 运行贝叶斯优化
X, y, model = bayesian_optimization(
    black_box_function, bounds, n_init=5, n_iter=20, xi=0.01
)
...
for poison, theta, reward in zip(poison_list[1:], theta_list[1:], reward_list[1:]):
    plt.text(
        theta,
        reward + 3,
        "$\sigma={}$ \n $({:.2f}, {:.2f})$".format(poison, reward, theta),
...
plt.text(
    ...
    reward_list[0] - 5,
    "$\sigma={}$ \n $({:.2f}, {:.2f})$".format(
        poison_list[0], reward_list[0], theta_list[0]
    ),
    fontproperties="Times New Roman",
    size=12,
)
