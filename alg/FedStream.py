...
args = {
    ...
    'batch_size': 32,
    'eval_freq': 5,
    'pre_estimate_path': '../logs/fedstream/pre_estimate',
    ...
    # 'con_4': 0.98,
    'poison_sigma': 0.25,
    'save_path_1': '../logs/fedstream/1',
    'save_path_2_delta': '../logs/fedstream/2_delta_model=fmnistcnn2',
    'save_path_2_data': '../logs/fedstream/2_data_model=fmnistcnn2',
    'save_path_2_stale': '../logs/fedstream/2_stale=fmnistcnn2',
    'save_path_2_cost': '../logs/fedstream/2_cost=fmnistcnn2',
    'save_path_3': '../logs/fedstream/3=fmnistcnn2',
    'save_path_4_loss': '../logs/fedstream/4_loss_fmnist_cnn_2=fmnistcnn2',
    'save_path_4_acc': '../logs/fedstream/4_acc_fmnist_cnn_2=fmnistcnn2',
...
class Server(object):
    ...
    def online_train(self):
        # 正式训练前定好一切
        pre_estimate_path_1 = self.pre_estimate_path + '_phi_best_sigma:{}.npy'.format(self.sigma)
        pre_estimate_path_2 = self.pre_estimate_path + '_theta_best_sigma:{}.npy'.format(self.sigma)
        ...
        for idx in range(len(enum)):
            ...
            pre_estimate_path_3_a = self.pre_estimate_path + '_a_theta_ver:{}_sigma:{}.npy'.format(str(round(new_theta, 2)), self.sigma)
            pre_estimate_path_3_b = self.pre_estimate_path + '_b_theta_ver:{}_sigma:{}.npy'.format(str(round(new_theta, 2)), self.sigma)
            pre_estimate_path_3_c = self.pre_estimate_path + '_c_theta_ver:{}_sigma:{}.npy'.format(str(round(new_theta, 2)), self.sigma)
            pre_estimate_path_3_d = self.pre_estimate_path + '_d_theta_ver:{}_sigma:{}.npy'.format(str(round(new_theta, 2)), self.sigma)
            ...
            accuracy_list = []
            # 计算聚合权重
            rate_matrix = np.stack([data_matrix[:, t] / sum(data_matrix[:, t]) for t in range(self.num_round)])
            rate_matrix = torch.from_numpy(rate_matrix).T
                                
            # 训练
            for t in tqdm(range(self.num_round)):
                # 并行但每个任务太轻，还不够任务切换的时间开销
                # # 定义并行调用的包装函数
                # def parallel_local_update(client, 
                #                         idx, 
                #                         t, 
                #                         k, 
                #                         sigma, 
                ...
                #                         data_size, 
                #                         poison_sigma):
                #     return client.local_update(idx, 
                #                             t, 
                #                             k, 
                #                             sigma, 
                #                             num_epoch, 
                ...
                # # 构建参数打包列表（为每个 client 配置对应的参数）
                # client_tasks = []
                # for k in range(self.num_client):
                #     client = self.client_group.clients[k]
                #     task = (
                #         client,
                #         idx,
                #         t,
                ...
                #         self.global_parameter,
                #         new_theta,
                #         increment_matrix[k][t],           # 每个客户端独有
                #         data_matrix[k][t],           # 每个客户端独有
                #         args['poison_sigma']
                #     )
                ...
                # # 启动多线程执行
                # train_params = []
                # with ThreadPoolExecutor(max_workers=min(len(client_tasks), os.cpu_count() // 2)) as executor:
                #     futures = [executor.submit(parallel_local_update, *task) for task in client_tasks]
                #     for future in futures:
                #         train_params.append(future.result())
                        
                # # 开始聚合
                # next_global_parameter = {}
                # global_loss = 0
                # for k in range(self.num_client):
                #     local_parameter = train_params[k][0]
                #     for item in local_parameter.items():
                #         if item[0] not in next_global_parameter.keys():
                #             next_global_parameter[item[0]] = rate_matrix[k][t] * item[1]
                #         else:
                #             next_global_parameter[item[0]] += rate_matrix[k][t] * item[1]
                            
                #     local_loss = train_params[k][2]
