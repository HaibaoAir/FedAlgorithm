...
class ClientUpdate(object):
    ...
    def get_model(
        ...
        if self.args.model == "cnn" and (
            self.args.dataset == "cifar_LDA" or self.args.dataset == "cifar_rate"
        ):
            print(
                "-----model:{}   dataset:{}".format(self.args.model, self.args.dataset)
            )
            global_net = CNNCifar().to(device)
            clients_net = [CNNCifar().to(device) for _ in range(self.args.num_clients)]
        elif self.args.model == "lenet" and (
            self.args.dataset == "cifar_LDA" or self.args.dataset == "cifar_rate"
        ):
            print(
                "-----model:{}   dataset:{}".format(self.args.model, self.args.dataset)
        ...
        elif self.args.model == "cnn" and (
            self.args.dataset == "mnist_LDA" or self.args.dataset == "mnist_rate"
        ):
            print(
                "-----model:{}   dataset:{}".format(self.args.model, self.args.dataset)
            )
        ...
        elif self.args.model == "mobilenet" and (
            self.args.dataset == "cifar100_rate" or self.args.dataset == "cifar100_LDA"
        ):
            print(
                "-----model:{}   dataset:{}".format(self.args.model, self.args.dataset)
            )
            ...
            clients_net = [
                MobileNetV2(num_classes=self.args.num_classes).to(device)
        ...
        optimizer = [
            torch.optim.Adam(model.parameters(), lr=self.args.lr)
            for model in clients_net
        ]
    ...
    def get_moon_model(
        ...
        if self.args.model == "cnn" and self.args.dataset == "cifar_LDA":
            ...
            clients_net = [
                MOONCNNCifar().to(device) for _ in range(self.args.num_clients)
            ]
            ...
            self.args.dataset == "cifar_rate" or self.args.dataset == "cifar_LDA"
        ):
            print(
                "-----model:{}   dataset:{}".format(self.args.model, self.args.dataset)
            )
            ...
            clients_net = [
                MOONMobileNetV2().to(device) for _ in range(self.args.num_clients)
            ]
        else:
    ...
    def client_train_FedDyn(
        self, clients_net, global_net, optimizer, train_loader, prev_grads
    ):
        clients_net.train()
        epoch_loss = []
        ...
        for i in range(self.args.train_ep):
            ...
            for batch_index, (data, target) in enumerate(train_loader):
                ...
                curr_params = torch.cat(
                    [p.reshape(-1) for p in clients_net.parameters()]
                )
                lin_penalty = torch.sum(curr_params * prev_grads)

                norm_penalty = (self.args.feddyn_alpha / 2.0) * torch.linalg.norm(
                ...
                torch.nn.utils.clip_grad_norm_(
                    parameters=clients_net.parameters(), max_norm=10
                )
        ...
        for iter in range(self.args.train_ep):
            ...
            for batch_idx, (images, labels) in enumerate(train_loader):
                ...
                for w in local_weights:
                    # line 10 in algo
                    local_weights[w] = local_weights[w] - self.args.lr * (
    ...
    def client_train_FedProx(self, clients_net, global_net, optimizer, train_loader):
        ...
        for i in range(self.args.train_ep):
            ...
            for batch_index, (data, target) in enumerate(train_loader):
                ...
                loss = (
                    self.loss_func(output, target) + (self.args.mu / 2) * proximal_term
                )
                loss.backward()
                optimizer.step()
                batch_loss.append(loss.item())
    ...
    def client_train_moon(
        self, clients_net, global_net, optimizer, train_loader, previous_net
    ):
        clients_net.train()
        epoch_loss = []
    ...
    def FedAvg(self, global_net, clients_net):
        ...
        for k in globel_dict.keys():
            globel_dict[k] = torch.stack(
                [
                    clients_net[i].state_dict()[k].float()
    ...
    def split_data(self, index):
        ...
        client_loader = DataLoader(
            client_split, batch_size=self.args.train_bs, shuffle=True
        )
    ...
    def shake_split_data(self, index):
        ...
        client_loader = DataLoader(
            DatasetSplit(data_set.train, train_dict),
