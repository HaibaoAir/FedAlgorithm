...
import torch
import torchvision
from torch.utils.data import Dataset, DataLoader # 两件套
...
class Client(object):
    ...
    def pretrain_local_update(self,
                              num_epoch,
                              batch_size,
                              net,
                              criterion,
                              ...
                              weight, # 论文中的\alpha
                              round):
        net.load_state_dict(global_parameters_list[round], strict=True)
        ...
        for epoch in range(num_epoch):
            ...
            for param_group in optim.param_groups:
                grad = param_group['params'][0].grad.reshape(-1)
                key = param_group['params_name']
                w_round = global_parameters_list[round][key].reshape(-1)
                w_next_round = global_parameters_list[round + 1][key].reshape(-1)

                item1 += grad.dot(w_round - w_next_round) # 拉长成一维很常见，便于计算
                item2 += grad.dot(grad)
            lr = weight * item1 / (1 - weight + weight * item2)
            if lr < 0:
                lr = min_lr
            for param_group in optim.param_groups:
                param_group['lr'] = lr
...
# group = Client_Group(10, 'mnist', 0, 1)