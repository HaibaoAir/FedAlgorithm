...
args = {
    ...
    'pre_estimate_path': '../../logs/fedstream/pre_estimate_4.npy',
...
class Server(object):
    ...
    def estimate_direct_phi(self):
        ...
        # 计算新的phi_list[T]
        for idc in range(self.fix_max_iter):
            increment_matrix, data_matrix, stale_matrix = self.estimate_direct_D(phi_list, self.reward, self.theta)
            # print('******************************************************')
    ...
    def online_train(self):
        ...
        # 正式训练前定好一切
        if os.path.exists(self.pre_estimate_path) == False:
            phi_list = self.estimate_phi() # [T]
            result = self.estimate_reward_theta(phi_list) # [K, T]
            np.save(self.pre_estimate_path, result)
                
        result = np.load(self.pre_estimate_path)
        reward = result[0][0]
        theta = result[0][1]
        res = result[1]
        ...
        res_list = []
        acc_list = []
        # for delta_com in [[-40, 0], [-20, 0], [0, -0.3], [0, -0.2], [0, 0], [0, 0.2], [20, 0]]:
        for delta_com in [[-45, 0], [-30, 0], [-15, 0], [0, 0], [15, 0], [30, 0], [45, 0]]:
        # for delta_com in [[0, -0.15], [0, -0.1], [0, -0.05], [0, 0], [0, 0.2], [0, 0.3], [0, 0.4]]:
        # for delta_com in [[-30, -0.2], [-20, -0.2], [-10, -0.1], [0, 0], [10, 0.1], [20, 0.2], [30, 0.3]]:
