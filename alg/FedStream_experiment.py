...
class Server(object):
    ...
    def estimate_reward_theta_PSO(self, phi_list):
    ...
    def check_bo_convergence(self, res, N, eps_cost):
        ...
        # 最近 N 个目标函数值（U）
        if len(func_vals) >= N:
            recent_vals = func_vals[-N:]
            max_u, min_u = np.max(recent_vals), np.min(recent_vals)
            diff_u = max_u - min_u
            print(
                f"[U变化] 最近{N}轮 U 最大值={max_u:.4f}, 最小值={min_u:.4f}, 差值={diff_u:.4e}"
            )
            if diff_u < eps_cost:
                print("[判断] 目标函数 U 收敛")
        ...
        #     print(f"[参数变化] reward 波动={std_x[0]:.4e}, theta 波动={std_x[1]:.4e}")
        #     if std_x.max() < eps_param:
        #         print("[判断] 参数点已稳定")

    def estimate_reward_theta_BO(self, phi_list):
        ...
        # reward, theta = result.x
        # print(f"[BayesOpt] Best reward: {reward:.4f}, Best theta: {theta:.4f}")
        # self.check_bo_convergence(res=result, N=5, eps_cost=1e-3)
        # for reward, theta in result.x_iters:
        #     print(f"[BayesOpt] reward: {reward:.4f}, theta: {theta:.4f}")
    ...
    def estimate_phi(self):
        ...
        # 计算新的phi_list[T]
        for idc in range(self.fix_max_iter):
            var, res = self.estimate_reward_theta_BO(phi_list)
            reward, theta = var
    ...
    def online_train(self):
        ...
        exit(0)
        result = self.estimate_reward_theta_BO(phi_list)  # [K, T]
